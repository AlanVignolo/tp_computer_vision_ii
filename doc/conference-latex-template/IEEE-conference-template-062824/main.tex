...
\subsubsection{UNet++}
UNet++~\cite{zhou2018unetplusplus} reemplaza los saltos simples de U-Net por una red densa y anidada de subredes convolucionales interconectadas. Este diseño reduce la brecha semántica (\textit{semantic gap}) entre los mapas de características del codificador (\textit{encoder}) y el decodificador (\textit{decoder}) antes de la concatenación. La experimentación formal se centró en comparar una variante ligera D2 con una variante profunda D5, manteniendo constante el codificador y el mecanismo de atención al estudiar específicamente el efecto de la profundidad.

La configuración utilizada fue la siguiente:

\begin{itemize}
    \item \textbf{Codificador:} ResNet-34 preentrenado en ImageNet.
    \item \textbf{Profundidad del decodificador:} D2 y D5.
    \item \textbf{Atención:} bloques \textit{spatial and channel squeeze and excitation} (scSE) en el decodificador.
    \item \textbf{Implementación:} librería \texttt{segmentation\_models\_pytorch}.
\end{itemize}

D2 se considera la configuración ligera, orientada a reducir la complejidad y la latencia, mientras que D5 representa la configuración de mayor capacidad, orientada a maximizar la recuperación de detalles finos. La comparación experimental y las conclusiones se centran exclusivamente en estas dos configuraciones. La selección de distintos codificadores se fundamenta en trabajos previos que muestran que la capacidad y la familia del codificador influyen en la calidad de la segmentación de grietas~\cite{konig2021optimized}.
