...
El monitoreo periódico de la infraestructura civil (puentes, autopistas, túneles y edificaciones) es indispensable para preservar su integridad y prolongar su vida útil. Las grietas y microfisuras en el hormigón son los primeros síntomas visibles de degradación estructural por fatiga, asentamientos o factores ambientales; detectarlas de forma incipiente permite un mantenimiento preventivo que mitiga el riesgo de colapso y reduce los costos de reparación. Históricamente, esta tarea ha dependido de la inspección visual manual, un enfoque lento, costoso, subjetivo y peligroso en estructuras de gran altura o acceso restringido~\cite{perry2020streamlined}.

El avance del aprendizaje profundo (\textit{deep learning}) ha posibilitado automatizar esta inspección~\cite{liu2025crack}. A diferencia de la clasificación de imágenes o la detección con cajas delimitadoras, la segmentación semántica resulta indispensable en este dominio: al clasificar cada píxel (grieta versus fondo) permite caracterizar la geometría de la fisura (longitud, espesor, conectividad y evolución temporal), insumos básicos para el cálculo de índices de daño estructural.

La viabilidad práctica de estos sistemas ha sido validada en pruebas de campo recientes. Perry et al.~\cite{perry2020streamlined} (2020) desplegaron con éxito un sistema de inspección del puente \textit{Oxbow Bridge} (Colorado, EE.UU.) integrando UAVs, fotogrametría y redes convolucionales. Liu et al.~\cite{liu2025crack} (2025) cuantificaron grietas en 3D sobre un puente en arco real con errores inferiores al 2\%. Estos casos demandan no solo precisión sino procesamiento en tiempo real o embebido, lo que justifica optimizar modelos específicos para grietas, como muestran Zhang et al.~\cite{zhang2025accuracy} (2025) al ajustar una arquitectura YOLOv8 para una detección más precisa, rápida y liviana.
...
\textbf{Eficiencia computacional:} se evaluó la latencia de inferencia y el rendimiento en cuadros por segundo (\textit{Throughput}, $\text{FPS} = 1000 / \text{latencia}$) con \textit{batch size} unitario ($BS = 1$) a $512\times 512$. Para garantizar mediciones libres de sobrecargas de I/O o visualización en CPU, se implementó un protocolo estandarizado con eventos de GPU (\texttt{torch.cuda.Event}) y sincronización estricta (\texttt{torch.cuda.synchronize()}), descartando las primeras 30 iteraciones de calentamiento (\textit{warm-up}) y promediando sobre 150 iteraciones independientes.
...
\midrule
UNet++ & ResNet-34 + scSE (5 capas) & 0.8574 & 0.7288 & 0.8431 & 0.8457 & \textbf{0.8405} & $\approx 45.29$ & $\approx 22.08$ \\
UNet++ & ResNet-34 (5 capas, base) & 0.8524 & 0.7187 & 0.8363 & \textbf{0.8828} & 0.7945 & $\approx 27.48$ & $\approx 36.39$ \\
\textbf{UNet++} & EfficientNet-B0 + scSE (5 capas) & \textbf{0.8595} & \textbf{0.7324} & \textbf{0.8456} & 0.8777 & 0.8157 & $\approx 38.49$ & $\approx 25.98$ \\
\midrule
...
\begin{enumerate}
    \item \textbf{Desempeño de UNet++ con EfficientNet-B0 y scSE:} esta configuración alcanzó el mejor desempeño de segmentación, con un $IoU_{\text{grieta}}$ de $0.7324$, un $mIoU$ de $0.8595$ y un $F1\text{-Score}$ de $0.8456$, a una latencia de aproximadamente $38.49$~ms ($25.98$~FPS). La variante con ResNet-34 y scSE quedó muy cerca ($IoU$ de $0.7288$, $mIoU$ de $0.8574$), mientras que la configuración base sin scSE alcanzó $0.7187$, confirmando el aporte de la atención scSE.
...
\begin{enumerate}
    \item \textbf{Fidelidad de segmentación semántica:} UNet++ con codificador EfficientNet-B0 y bloques de atención espacial y de canal (scSE) demostró el desempeño más alto en precisión a nivel de píxel ($IoU_{\text{grieta}} = 0.7324$, $mIoU = 0.8595 $), confirmando la superioridad de las conexiones densas anidadas para preservar la continuidad en fisuras estructurales complejas.
    ...
    \item \textbf{Eficacia de la supervisión de contornos (PIDNet):} La incorporación de una rama derivativa de bordes explícita modulada mediante pérdida \textit{Boundary-Aware} permitió a PIDNet-S alcanzar un $IoU_{\text{grieta}}$ de $0.6660$ con una latencia de solo $11.62$~ms ($86.06$~FPS). Esto representa una reducción aproximada del $69.8\%$ en la latencia y una velocidad de inferencia aproximadamente $3.31$ veces superior respecto a UNet++ con EfficientNet-B0 y atención scSE, cuya latencia fue de $38.49$~ms ($25.98$~FPS). Estos resultados evidencian la efectividad de guiar la segmentación de estructuras delgadas mediante información de alta frecuencia.
